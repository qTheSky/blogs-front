...
export const blogsAPI = {
  ...
  async getPostsOfBlog(blogId: string) {
    return await axiosInstance.get<IPaginatorWithItems<IPost[]>>(
      `/blogs/${blogId}/posts`
    );
  },
  ...
  // public api
  // bloggers api
  async getAllCommentsForBlogger() {
    // todo add query
    return await axiosInstance.get<IPaginatorWithItems<IComment[]>>(
  ...
  async getBannedUsersInBlog(blogId: string) {
    return await axiosInstance.get<IPaginatorWithItems<IBannedUserInBlog[]>>(
